...
pt::ptree load(const std::string& filename = "short_vs_all.xml")
{
  pt::ptree tree;
  ...
  for (const pt::ptree::value_type& iteration: tree.get_child("BlastOutput.BlastOutput_iterations"))
  {
    ...
    std::clog << iteration.second.get<std::string>("Iteration_iter-num") << '\n';
    //for (const pt::ptree::value_type& iteration: blast_output.second.get_child("Iteration"))
    //{
    //  std::clog << iteration.second.get<std::string>("Iteration_iter_name") << '\n';
    //}
  }
  /*
